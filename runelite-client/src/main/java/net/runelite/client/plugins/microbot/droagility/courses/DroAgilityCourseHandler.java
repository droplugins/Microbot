...
public interface DroAgilityCourseHandler
{

	WorldPoint getStartPoint();

	List<DroAgilityObstacleModel> getObstacles();
	...
	default int getClientPlane()
	{
		return Microbot.getClientThread().runOnClientThreadOptional(() -> {
			...
			return Microbot.getClient().getTopLevelWorldView().getPlane();
		}).orElse(-1);
	}

	default int getAgilityXp()
	{
		return Microbot.getClientThread().runOnClientThreadOptional(() ->
			Microbot.getClient().getSkillExperience(Skill.AGILITY)).orElse(0);
	}
	...
	default TileObject getCurrentObstacle()
	{
		...
		if (matchingObstacles.isEmpty())
		{
			int fallbackIndex = getCurrentObstacleIndex();
			if (fallbackIndex >= 0 && fallbackIndex < getObstacles().size())
			{
				matchingObstacles = List.of(getObstacles().get(fallbackIndex));
			}
		}
	...
	default int getCurrentObstacleIndex()
	{
		...
		{
			DroAgilityObstacleModel o = getObstacles().get(i);
