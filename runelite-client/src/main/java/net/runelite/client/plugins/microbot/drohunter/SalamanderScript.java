...
public class SalamanderScript extends Script {
    ...
    private SalamanderHunting getSalamander(SalamanderConfig config) {
        if (config.salamanderProgressiveHunting()) {
    ...
    private boolean handleExistingTraps(SalamanderPlugin plugin, SalamanderConfig config) {
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            ...
            if (gameObject != null) {
                ...
                SalamandersCaught++;
                sleep(config.salamanderMinSleepAfterCatch(), config.salamanderMaxSleepAfterCatch());
                return true;
            }
        }
        return false;
    ...
    private void setNewTrap(SalamanderHunting salamanderType, SalamanderConfig config) {
        if (Microbot.getRs2TileObjectCache().query().withId(salamanderType.getTreeId()).count() > 0) {
            ...
            sleep(config.salamanderMinSleepAfterLay(), config.salamanderMaxSleepAfterLay());
        }
    }
