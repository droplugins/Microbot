...
public class HunterKebbitsOverlay extends OverlayPanel {
    ...
    private final DroHunterConfig config;
     ...
     */
    @Inject
    public HunterKebbitsOverlay(Client client, DroHunterConfig config, HunterKebbitsPlugin plugin, HunterKabbitsScript script) {
        super(plugin);
        this.client = client;
        this.config = config;
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        panelComponent.getChildren().add(LineComponent.builder()
                .left("Bury Bones:")
                .right(config.kebbitBuryBones() ? "Yes" : "No")
                .build());
