...
@Slf4j
public class DroBrimhavenSpikeCourse implements DroAgilityCourseHandler {
    ...
    public List<DroAgilityObstacleModel> getObstacles() {
        return List.of(
            new DroAgilityObstacleModel(3566), // Brimhaven spike obstacle 1 (rope swing)
            new DroAgilityObstacleModel(3578), // Brimhaven spike obstacle 2 (pillar jump)
            new DroAgilityObstacleModel(9999)  // Brimhaven spike obstacle 3 (spikes - placeholder, handled by timed tile-walking)
        );
    }
    ...
    public net.runelite.api.TileObject getCurrentObstacle() {
        ...
        if (playerLocation.getPlane() == 3) {
            ...
            List<DroAgilityObstacleModel> obstacles = getObstacles();
            if (currentObstacleIndex < obstacles.size()) {
                DroAgilityObstacleModel currentObstacle = obstacles.get(currentObstacleIndex);
