...
import net.runelite.client.plugins.microbot.droagility.enums.DroAgilityEquipmentOption;
import net.runelite.client.plugins.microbot.droagility.enums.DroAgilityFoodOption;
import net.runelite.client.plugins.microbot.droagility.enums.DroAgilityPotionOption;
...
class DroAgilitySupplyManager
{
	...
	private final DroAgilityPlugin plugin;
	private final DroAgilityConfig config;
	...
	DroAgilitySupplyManager(DroAgilityPlugin plugin, DroAgilityConfig config, BooleanSupplier shuttingDown, Runnable shutdown)
	{
		this.plugin = plugin;
		this.config = config;
	...
	boolean handleSummerPies(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex, InventorySnapshot inventorySnapshot)
	{
	...
	boolean handlePreLevelCheck(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex,
	...
	boolean handleBeforeObstacle(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex,
	...
	boolean shouldWalkToCourseStartForSummerPie(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex)
	{
	...
	private boolean handleBanking(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex,
								  InventorySnapshot inventorySnapshot)
	{
								  ...
								  InventorySnapshot inventorySnapshot, boolean blockUntilBankable)
	{
		...
		int bankEpoch = Rs2Bank.getBankLiveEpoch();
		boolean bankOpen = bankWasOpen || Rs2Bank.walkToBankAndUseBank();
		if (!bankOpen)
		{
		...
		if (!Global.sleepUntil(() -> Rs2Bank.verifyBankMirrorAfterOpen(bankWasOpen, bankEpoch), 4000))
		{
			lastBankFailureAt = System.currentTimeMillis();
			Microbot.log("Bank opened, but its item data was not ready. Retrying after cooldown.");
			Rs2Bank.closeBank();
			return true;
		}
	...
	private boolean withdrawConfiguredSupplies(DroAgilityCourseHandler courseHandler)
	{
	...
	private boolean withdrawConfiguredFood()
	{
		...
		DroAgilityFoodOption option = config.bankFood();
		if (option.isAuto())
		{
	...
	private boolean withdrawAutoFood(int amountToWithdraw, int currentFood)
	{
		...
		List<DroAgilityFoodOption> foodOptions = Arrays.stream(DroAgilityFoodOption.values())
			.filter(foodOption -> !foodOption.isNone() && !foodOption.isAuto())
			.filter(foodOption -> foodOption.getFood() != null)
			.sorted(Comparator.comparingInt((DroAgilityFoodOption foodOption) -> foodOption.getFood().getHeal()).reversed())
			.collect(Collectors.toList());

		for (DroAgilityFoodOption option : foodOptions)
		{
	...
	{
		DroAgilityPotionOption option = config.bankPotion();
	...
	private boolean shouldBankForSupplies(DroAgilityCourseHandler courseHandler, InventorySnapshot inventorySnapshot)
	{
	...
	{
		DroAgilityFoodOption food = config.bankFood();
		return !food.isNone()
	...
	{
		DroAgilityPotionOption potion = config.bankPotion();
		return !potion.isNone()
			&& config.potionWithdrawAmount() > 0
			&& inventorySnapshot.getPotionCount() < config.potionWithdrawAmount();
	}

	private boolean needsSummerPieBanking(DroAgilityCourseHandler courseHandler, InventorySnapshot inventorySnapshot)
	{
	...
	private int getBankPotionCount(DroAgilityPotionOption potion)
	{
	...
	private boolean withdrawPotionFamily(DroAgilityPotionOption potion, int amount)
	{
	...
	private boolean canAttemptBanking(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex)
	{
	...
	private boolean isAtCourseStart(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex)
	{
		return courseHandler != null
	...
	private boolean shouldEatSummerPie(DroAgilityCourseHandler courseHandler, WorldPoint playerWorldLocation, int currentObstacleIndex)
	{
		if (isAtCourseStart(courseHandler, playerWorldLocation, currentObstacleIndex))
		{
			return true;
	...
	private boolean shouldUseSummerPies(DroAgilityCourseHandler courseHandler)
	{
